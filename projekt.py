...
class Program:
    ...
    def __init__(self, run=True, load_images=True):
        ...
        self.player2.find_moves(self.player1.pieces)
        self.player1.find_moves(self.player2.pieces)
        ...
        self.canvas = tkinter.Canvas(root, height=self.SQUARE_SIZE * 8 + 2 * self.SPACER,
                                     width=self.SQUARE_SIZE * 8 + 3 * self.SPACER + self.game_size, bg="black")
    ...
    def load_images(self):
        ...
        resizer = 0.3
        flag = Image.open('flag.png').resize((int(resizer * self.SQUARE_SIZE), int(resizer * self.SQUARE_SIZE)))
        half = Image.open('half.png').resize((int(resizer * self.SQUARE_SIZE), int(resizer * self.SQUARE_SIZE)))
    ...
    def board(self):
        ...
        self.generate_game_report_space()

        self.generate_best_players_space()
    ...
    def generate_best_players_space(self):
        self.generate_side_panel("BEST PLAYERS", self.best_players, 3 * self.SPACER + self.game_size)

    def generate_game_report_space(self):
        self.generate_side_panel("GAME REPORT", self.notation, 2 * self.SPACER)

    def generate_side_panel(self, title, content, x_offset):
        left = self.SQUARE_SIZE * 8 + x_offset
        right = left + self.game_size
    ...
    def generate_pieces_on_canvas(self):
        piece_parameters = [(self.player1, self.w_pawn, self.w_queen, "w"),
                            (self.player2, self.b_pawn, self.b_queen, "b")]
    ...
    def add_to_notation_history(self, new_move_notation):
        length_moves = len(self.moves)

        if length_moves % self.BOUND_OF_GAME_REPORT == 0 and length_moves != 0:
            self.notation += "\n"

        if length_moves % 2 == 0:
            self.notation += f"{length_moves // 2 + 1}. "
    ...
    def offer_or_accept_draw(self, player):
        if ((player == self.player1 and self.player2.offering_draw) or
            ...
            self.set_game_result(0)  # Draw accepted
        else:
    ...
    def set_game_result(self, result):
        ...
        self.player1.update_rating(result, self.player2.rating[0])
        self.player2.update_rating(result * (-1), self.player1.rating[0])
